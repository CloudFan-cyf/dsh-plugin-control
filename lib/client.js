...
const css = [
  ...
  ".dpc_controlBadge{background:color-mix(in srgb,var(--dsw-alias-state-business-primary) 12%,transparent);color:var(--dsw-alias-state-business-primary);border-radius:999px;padding:1px 8px;font-size:11px;line-height:17px;white-space:nowrap}",
...
const zh = {
  ...
  typeBuiltin: '内置模块',
  controlMode: '控制方式',
  controlSettings: '设置开关',
  controlLoader: 'Loader 开关',
  notProvided: '未提供',
  related: '同包条目',
  phasePending: '等待依赖',
  phaseLoading: '加载中',
...
const en = {
  ...
  controlSettings: 'Settings switch',
  controlLoader: 'Loader switch',
...
function PluginControlSection(props) {
    ...
    return h('li', {
      ...
      h('div', { className: styles.cardHead },
        ...
        h('span', { className: styles.badges },
          entry.settingsControlled ? h('span', { className: styles.controlBadge }, t('controlSettings')) : null,
  ...
  const renderDetail = (entry) => {
    ...
    const meta = [
      ...
      [t('controlMode'), entry.settingsControlled ? t('controlSettings') : t('controlLoader')],
    ]
