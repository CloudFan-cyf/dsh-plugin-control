...
import { applySwitchValue, settingsSwitchFor } from './settings-switches.js'
...
export function describeEntries(ctx, profileDir) {
  const settingsDescriptors = ctx.settings?.describe ? ctx.settings.describe() : []
  const namespaces = new Map(settingsDescriptors.map((descriptor) => [String(descriptor.ns), descriptor]))
  const entries = []
  for (const entry of ctx.loader.entries()) {
    ...
    const settingsDescriptor = sw === null ? undefined : namespaces.get(sw.namespace)
    const settingsEnabled = sw !== null && settingsDescriptor !== undefined
      ? applySwitchValue(entryId, settingsDescriptor.value)
      : undefined
    entries.push({
      entryId,
      runtimeEntryId,
      ...
      enabled: settingsEnabled === undefined ? entry.disabled !== true : settingsEnabled !== false,
      ...
      settingsControlled: settingsEnabled !== undefined,
      settingsNamespace: sw === null ? null : sw.namespace,
      settingsField: sw === null ? null : sw.field,
    })
  }
  entries.sort((a, b) => {
