...
import { buildSwitchOps, settingsSwitchFor } from './settings-switches.js'
...
export function makeRoutes({ ctx, profileDir }) {
  ...
  /** Upsert the loader-level `disabled` patch for one entry (atomic write). */
  const persistLoaderPatch = (entryId, enabled) => {
    const currentText = existsSync(patchPath) ? readFileSync(patchPath, 'utf8') : ''
    const currentEntries = parsePatchFile(currentText)
    const nextEntries = applyToggle(currentEntries, entryId, enabled)
  ...
  const findSettingsDescriptor = (namespace) => {
    if (ctx.settings?.describe === undefined) return undefined
    return ctx.settings.describe().find((descriptor) => String(descriptor.ns) === namespace)
  }

  const waitForSettingsNamespace = (namespace) => {
    const deadline = Date.now() + POLL_TIMEOUT_MS
    return new Promise((resolve) => {
      const tick = () => {
        if (findSettingsDescriptor(namespace) !== undefined) {
          resolve(true)
          return
        }
  ...
  const mutateSettings = async (sw, entryId, enabled) => {
    if (ctx.settings?.mutate === undefined) throw new Error('settings service unavailable')
    await ctx.settings.mutate(sw.namespace, buildSwitchOps(entryId, enabled))
  }
  ...
  const routes = [
    ...
    },
    {
      ...
      handler: async (req, res) => {
        ...
        try {
          const sw = settingsSwitchFor(body.entryId)
          const applied = await enqueueWrite(async () => {
            if (sw !== null) {
              // Settings-controlled plugins (e.g. dsh-pet) gate their visible
              // surface through their own settings namespace. Toggling the
              // Loader entry alone would leave the client UI mounted while
              // the host routes disappear. Instead keep the host mounted and
              // flip the owning settings field.
              const descriptor = findSettingsDescriptor(sw.namespace)
              if (!body.enabled || descriptor === undefined) {
                // Make sure the host is mounted first so the settings
                // namespace is registered and the switch is authoritative.
                persistLoaderPatch(body.entryId, true)
                await waitForEntryState(body.entryId, true)
                await waitForSettingsNamespace(sw.namespace)
              }
              await mutateSettings(sw, body.entryId, body.enabled)
              return true
            }
            persistLoaderPatch(body.entryId, body.enabled)
